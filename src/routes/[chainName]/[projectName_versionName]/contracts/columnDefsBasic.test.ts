...
describe("columnDefsBasic", () => {
  test("should link the contract name to the first tab of the contract", () => {
    const columnDef = columnDefsBasic(
      "/digu/ethereum/Uniswap-v3/contracts/",
    ) as ColGroupDef;
    ...
    expect(vi.mocked(mount).mock.calls[0][1].props).toMatchObject({
      href: "/digu/ethereum/Uniswap-v3/contracts/Token#overview",
    });
  });
});
